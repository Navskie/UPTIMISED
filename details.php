...
<?php
    ...
    if (mysqli_num_rows($price_stmt) > 0) {
      $country_price = $price_fetch['country_price'];
    } else {
      $country_price = 0;
    }
?>
        ...
        <div class="row">
            ...
            <div class="col-sm-12 col-md-6 col-lg-6">
                            ...
                            <div class="col-sm-12 col-md-6 col-lg-6">
                                <p class="product-single__price product-single__price-product-template">
                                    ...
                                    <span class="product-price__price product-price__price-product-template product-price__sale product-price__sale--single">
                                        <span id="ProductPrice-product-template"><span class="money"><?php echo $country_code ?> <?php echo number_format($country_price) ?></span></span>
                                    </span>
                                </p>
                            </div>
                ...
                <form action="backend/add-to-cart.php?code=<?php echo $d_item_code ?>" onclick="window.location.href='cart.php'" method="post" class="item">
                    <?php
                        $main_code_qry = mysqli_query($connect, "SELECT * FROM upti_code WHERE code_name = '$item_code'");
                        $maincode = mysqli_fetch_array($main_code_qry);

                        $main_code = $maincode['code_main'];
                        
                        $pack_stock = mysqli_query($connect, "SELECT * FROM upti_pack_sett WHERE p_s_main = '$item_code'");
                    ...
                    ?>
                        ...
                        <?php } elseif ($loop_qty == $inv) { ?>
                            <button class="btn btn-custom w-100" tabindex="0" name="addtocart">ADD TO CART - <?php echo $country_code ?> <?php echo number_format($country_price); ?></button>
                    <?php } else { ?>
                        <button class="btn btn-custom w-100" tabindex="0" disabled>NOT AVAILABLE</button>
                    <?php } ?>
                </form>
