<?php
    ...
    if (isset($_POST['submit-product'])) {
        ...
        if (mysqli_num_rows($register_stmt) > 0) {
            $code_stmt = mysqli_query($connect, "SELECT * FROM upti_package WHERE package_code = '$p_code'");
            if (mysqli_num_rows($code_stmt) == 0) {
                ...
                $b_code = $_POST['b_code'];
                $b_qty = $_POST['b_qty'];
                $b2_code = $_POST['b2_code'];
                $b2_qty = $_POST['b2_qty'];
                $b3_code = $_POST['b3_code'];
                $b3_qty = $_POST['b3_qty'];
                $b4_code = $_POST['b4_code'];
                $b4_qty = $_POST['b4_qty'];
                $b5_code = $_POST['b5_code'];
                $b5_qty = $_POST['b5_qty'];
                ...
                flash("success", "Bundle items has been added successfully");
                header('location: ../creatives.php');
            } else {
                flash("failed", "Failed to add product, Item Code already exist!");
                header('location: ../creatives-bundle.php');
            }
        } else {
