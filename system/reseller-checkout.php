<?php
    ...
    if (isset($_POST['checkouts'])) { // isset if
      ...
      if ($img_name != '' && $mode_of_payment == 'E-Payment' || $mode_of_payment != 'E-Payment') { // img required
        ...
        $regular_sql = mysqli_query($connect, "SELECT SUM(ol_qty) AS regular FROM upti_order_list INNER JOIN upti_code ON code_name = ol_code WHERE ol_poid = '$poid' AND code_category = 'RESELLER'");
        ...
        if ($upsell > 0 && $regular == '' || $direct > 0 && $regular == '') { // wag mo ilabas tie up mo boi
          ...
          header('location: osr-reseller.php');
              ...
              if ($upsell <= $regular && $direct <= $regular && $regular == 1) { // one is to one upsell and direct sales
