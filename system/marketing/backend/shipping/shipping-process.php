<?php
    if (isset($_POST['shipping'])) {
        ...
        if ($check_num_row == 0) {
            $epayment_process = "INSERT INTO upti_shipping (shipping_country, shipping_price, shipping_status) VALUES ('$country', '$price', 'Active')";
            $epayment_process_qry = mysqli_query($connect, $epayment_process);

            echo "<script>alert('Data has been Added successfully.');window.location.href = 'shipping.php';</script>";
        } else {
            echo "<script>alert('Shipping fee in this country is already have.');window.location.href = 'shipping.php';</script>";
        }
