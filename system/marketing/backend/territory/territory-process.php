<?php
    if (isset($_POST['bansa'])) {
        ...
        $epayment_process = "INSERT INTO upti_territory (territory_name) VALUES ('$territory')";
        $epayment_process_qry = mysqli_query($connect, $epayment_process);

        echo "<script>alert('Data has been Added successfully.');window.location.href = 'territory.php';</script>";
    }
?>
        ...
        <form action="territory" method="post">
            <div class="row">
                <div class="col-12">
                    ...
                    <input type="text" class="form-control" name="territory" autocomplete="off" required>
                </div>
            </div>
        </div>
